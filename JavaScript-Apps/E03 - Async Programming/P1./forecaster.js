...
function findUpcoming(code) {
    $.get(`https://judgetests.firebaseio.com/forecast/upcoming/${code}.json`)
        .then(function (data) {
            ...
            //     .append(`<span class="upcoming"></span>`)
            // $(`span.upcoming`)
            //     .append(`<span class="symbol">${findSymbol(data.forecast[2].condition)}</span>`)
            //     .append(`<span class="forecast-data">${data.forecast[2].high}${findSymbol('Degrees')}/${data.forecast[2].low}${findSymbol('Degrees')}</span>`)
